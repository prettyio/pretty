...
var require_get_stream = __commonJS({
  'node_modules/get-stream/index.js'(exports, module2) {
    ...
    async function getStream2(inputStream, options) {
      ...
      await new Promise((resolve, reject) => {
        const rejectPromise = (error) => {
          if (
            error &&
            stream2.getBufferedLength() <= BufferConstants.MAX_LENGTH
          ) {
    ...
    module2.exports.buffer = (stream2, options) =>
      getStream2(
        stream2,
        __spreadProps(__spreadValues({}, options), { encoding: 'buffer' })
      );
    module2.exports.array = (stream2, options) =>
      getStream2(
        stream2,
        __spreadProps(__spreadValues({}, options), { array: true })
      );
...
function npmRunPath(options = {}) {
  ...
  let previous;
  const cwdString =
    cwd instanceof URL ? import_node_url.default.fileURLToPath(cwd) : cwd;
...
function npmRunPathEnv(_a = {}) {
  var _b = _a,
    { env = import_node_process.default.env } = _b,
    options = __objRest(_b, ['env']);
...
${fromBody}`;
var toStringDescriptor = Object.getOwnPropertyDescriptor(
  Function.prototype,
  'toString'
);
var toStringName = Object.getOwnPropertyDescriptor(
...
var changeToString = (to, from, name) => {
  ...
  Object.defineProperty(
    to,
    'toString',
    __spreadProps(__spreadValues({}, toStringDescriptor), {
      value: newToString,
    })
  );
};
...
var findSignalByNumber = function (number, signals) {
  const signal = signals.find(
    ({ name }) => import_os2.constants.signals[name] === number
  );
  if (signal !== void 0) {
    return signal;
  }
...
var makeError = ({
  ...
  const signalDescription =
    signal === void 0 ? void 0 : signalsByName[signal].description;
...
var isSigterm = (signal) =>
  signal === import_node_os.default.constants.signals.SIGTERM ||
...
var mergePromise = (spawned, promise) => {
  for (const [property, descriptor] of descriptors) {
    ...
    Reflect.defineProperty(
      spawned,
      property,
      __spreadProps(__spreadValues({}, descriptor), { value })
    );
  }
  return spawned;
};
...
var getEnv = ({
  ...
  const env = extendEnv
    ? __spreadValues(
        __spreadValues({}, import_node_process2.default.env),
        envOption
      )
    : envOption;
  if (preferLocal) {
...
var handleArguments = (file, args, options = {}) => {
  ...
  if (
    import_node_process2.default.platform === 'win32' &&
...
function execa(file, args, options) {
  ...
  let spawned;
  try {
    spawned = import_node_child_process.default.spawn(
      parsed.file,
      parsed.args,
      parsed.options
    );
...
function execaSync(file, args, options) {
  ...
  let result;
  try {
    result = import_node_child_process.default.spawnSync(
      parsed.file,
      parsed.args,
      parsed.options
    );
...
function execaNode(scriptPath, args, options = {}) {
  ...
  const defaultExecArgv = import_node_process2.default.execArgv.filter(
