    ...
    var fillNumbers = (start, end, step = 1, options = {}) => {
      ...
      if (options.toRegex === true) {
        return step > 1
          ...
          : toRegex(range, null, __spreadValues({ wrap: false }, options));
      }
      return range;
    };
        ...
        if (token.type === 'negate') {
          ...
          if (
            ...
            const expression = parse(
              rest,
              __spreadProps(__spreadValues({}, options), { fastpaths: false })
            ).output;
  ...
  'node_modules/micromatch/index.js'(exports, module2) {
    ...
    var micromatch = (list, patterns, options) => {
      ...
      for (let i = 0; i < patterns.length; i++) {
        let isMatch = picomatch(
          String(patterns[i]),
          __spreadProps(__spreadValues({}, options), { onResult }),
          true
        );
    ...
    micromatch.contains = (str, pattern, options) => {
      ...
      return micromatch.isMatch(
        str,
        pattern,
        __spreadProps(__spreadValues({}, options), { contains: true })
      );
    };
    ...
    micromatch.capture = (glob, input, options) => {
      ...
      let regex = picomatch.makeRe(
        String(glob),
        __spreadProps(__spreadValues({}, options), { capture: true })
      );
    ...
    micromatch.braceExpand = (pattern, options) => {
      ...
      return micromatch.braces(
        pattern,
        __spreadProps(__spreadValues({}, options), { expand: true })
      );
    };
    ...
    function readdir(directory, settings, callback) {
      settings.fs.readdir(directory, (readdirError, names) => {
        ...
        const tasks = names.map((name) => {
          const path2 = common.joinPathSegments(
            directory,
            name,
            settings.pathSegmentSeparator
          );
  ...
  'node_modules/dir-glob/index.js'(exports, module2) {
    ...
    var getGlob = (directory, options) => {
      ...
      if (options.files && options.extensions) {
        return options.files.map((x) =>
          path2.posix.join(directory, addExtensions(x, options.extensions))
        );
      }
      if (options.files) {
    ...
    module2.exports.sync = (input, options) => {
      ...
      const globs = []
        .concat(input)
        .map((x) =>
          pathType.isDirectorySync(getPath2(x, options.cwd))
            ? getGlob(x, options)
            : x
        );
      return [].concat.apply([], globs);
...
var require_ignore = __commonJS({
  'node_modules/ignore/index.js'(exports, module2) {
    ...
    var SLASH = '/';
    var KEY_IGNORE =
      typeof Symbol !== 'undefined' ? Symbol.for('node-ignore') : 'node-ignore';
    ...
    var isPathValid = (path2) =>
      checkPath(path2 && checkPath.convert(path2), path2, RETURN_FALSE);
    ...
    if (
      ...
      checkPath.isNotRelative = (path2) =>
        REGIX_IS_WINDOWS_PATH_ABSOLUTE.test(path2) || isNotRelative(path2);
    }
  },
});
...
var require_gitignore = __commonJS({
  'node_modules/globby/gitignore.js'(exports, module2) {
    ...
    var parseGitIgnore = (content, options) => {
      const base = slash(
        path2.relative(options.cwd, path2.dirname(options.fileName))
      );
      return content
        .split(/\r?\n/)
        .filter(Boolean)
    ...
    var getIsIgnoredPredecate = (ignores, cwd) => {
      return (p) =>
        ignores.ignores(
          slash(path2.relative(cwd, ensureAbsolutePathForCwd(cwd, p.path || p)))
        );
    };
...
var require_globby = __commonJS({
  'node_modules/globby/index.js'(exports, module2) {
    ...
    module2.exports.hasMagic = (patterns, options) =>
      []
        .concat(patterns)
  ...
  'node_modules/is-path-inside/index.js'(exports, module2) {
    ...
    module2.exports = (childPath, parentPath) => {
      ...
      return Boolean(
        relation &&
          relation !== '..' &&
          !relation.startsWith(`..${path2.sep}`) &&
          relation !== path2.resolve(childPath)
      );
    };
  },
});
...
var require_rimraf = __commonJS({
  'node_modules/rimraf/rimraf.js'(exports, module2) {
    ...
    var rmkidsSync = (p, options) => {
      ...
      options
        .readdirSync(p)
        .forEach((f) => rimrafSync(path2.join(p, f), options));
...
var require_del = __commonJS({
  'node_modules/del/index.js'(exports, module2) {
    ...
    module2.exports = async (patterns, _a = {}) => {
      var _b = _a,
        { force, dryRun, cwd = process.cwd() } = _b,
        options = __objRest(_b, ['force', 'dryRun', 'cwd']);
      options = __spreadValues(
        {
...
var generateRandomBytes = (byteLength, type, length) =>
  import_crypto.default.randomBytes(byteLength).toString(type).slice(0, length);
...
var createGenerator =
  (generateForCustomCharacters2, generateRandomBytes2) =>
...
var cryptoRandomString = createGenerator(
...
function isStream(stream2) {
  return (
    stream2 !== null &&
    typeof stream2 === 'object' &&
    typeof stream2.pipe === 'function'
  );
}
...
var getPath = (prefix = '') =>
  import_node_path.default.join(
    import_temp_dir.default,
    prefix + uniqueString()
  );
var writeStream = async (filePath, data) =>
  pipeline(data, import_node_fs.default.createWriteStream(filePath));
var createTask =
  (tempyFunction, { extraArguments = 0 } = {}) =>
  async (...arguments_) => {
    const [callback, options] = arguments_.slice(extraArguments);
    const result = await tempyFunction(
      ...arguments_.slice(0, extraArguments),
      options
    );
    try {
...
tempy.file = (options) => {
  ...
  return (
    getPath() +
    (options.extension === void 0 || options.extension === null
      ? ''
...
tempy.write = async (data, options) => {
  ...
  const write = isStream(data)
    ? writeStream
    : import_node_fs.promises.writeFile;
